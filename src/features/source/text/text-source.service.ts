...
class TextSourceService {
  public async getAllTextSources(userId: number): Promise<TextSource[]> {
    try {
      ...
      throw new HttpException(500, `Error fetching text sources: ${error.message}`);
    }
  }

  public async getTextSourceById(sourceId: number, trx?: typeof knex): Promise<TextSource> {
    try {
      ...
      throw new HttpException(500, `Error fetching text source: ${error.message}`);
    }
  }
  ...
  public async createTextSource(
    ...
    try {
      ...
      if (error instanceof HttpException) throw error;
      throw new HttpException(500, `Error creating text source: ${error.message}`);
    }
  }
    ...
    try {
      ...
      return await knex.transaction(async trx => {
        ...
        if (content !== undefined) {
          await trx('text_sources').where('source_id', sourceId).update({ content });
        }
      ...
      if (error instanceof HttpException) throw error;
      throw new HttpException(500, `Error updating text source: ${error.message}`);
    }
  }
}
