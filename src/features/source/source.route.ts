...
import { createSourceSchema, updateSourceSchema } from './file/source.validation';
...
class BaseSourceRoute implements Route {
  ...
  private initializeRoutes() {
    ...
    this.router.get(`${this.path}`, this.baseSourceController.getAllSourcesByUserId);
    this.router.post(
      `${this.path}`,
      validationMiddleware(createSourceSchema),
      this.baseSourceController.createSource
    );
    this.router.get(`${this.path}/:id`, this.baseSourceController.getSourceById);
    this.router.put(
      `${this.path}/:id`,
      validationMiddleware(updateSourceSchema),
      this.baseSourceController.updateSource
    );
    this.router.delete(`${this.path}/:id`, this.baseSourceController.deleteSource);
  }
}
